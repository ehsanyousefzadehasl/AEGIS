...
def build_model(device: torch.device) -> nn.Module:
    model = models.inception_v3(weights=None, aux_logits=False, num_classes=1000)
...
def estimate_model_memory_bytes(model: nn.Module, batch_size: int) -> int:
    ...
    def input_builder():
        return torch.rand(batch_size, 3, 299, 299, requires_grad=True).to("cuda")
...
def main() -> None:
    ...
    with timed_run() as total_timer:
        ...
        if args.print_model_summary or args.summary_output is not None:
            summary_input = torch.rand(args.batch_size, 3, 299, 299, device=device)
